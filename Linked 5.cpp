...
#include <iostream>
using namespace std;
//this function to definition to structure
struct Node {
	int data;
	struct Node *next;//added node new
};
struct Node* top = NULL;//rename node* top=NULL
void push(int val) {
...
struct node {
	...
	class list {
// the class and add public member to all functions available
		public:
...
//this function is for linked list and it is to add or delete
//this is to added Element at the begin
...
//this is to added Element at the last
...
//this is functions to delete Element at the list		
...
//this is to search Any Element in list
void list::search() {
	int value, pos = 0;
	bool flag = false;
	if (start == NULL) {
...
//this is functions to added Element in stack
void pop() {
	if (top == NULL){
...
//this is to insert Element in queue
void Insert() {
	int val;
	if (rear == n - 1)
	cout << "\nQueu overflow" << endl;
...
//this is to delete Element in gueue
void Delete() {
...
//this is to display the Elements in queue
void Display() {
	if (front == - 1)
	cout << "\nQueue is empty" << endl;
	else {
